...
AddEventHandler("chasse:createBlip", function(type, x, y, z)
    ...
    if(type == 141)then
        ...
        AddTextComponentString("[Activité] Chasse")
...
function VenteViande(qte)
    if qte > 0 then
        TriggerEvent('player:looseItem', 23, 1)
        local salaire = math.random(40, 60)
        TriggerServerEvent('mission:completed', salaire)
        ClearPrints()
        SetTextEntry_2("STRING")
        AddTextComponentString("~g~Vous avez vendu de la viande fraiche")
        DrawSubtitleTimed(2000, 1)
    else
        ClearPrints()
        SetTextEntry_2("STRING")
        AddTextComponentString("~r~Vous n'avez plus de viande fraiche")
        DrawSubtitleTimed(2000, 1)
    end
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if pos then
            ...
            -- end {960.196105957031, -2105.53198242188,  31.9526977539063, 91.5713500976563}
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 960.196105957031, -2105.53198242188, 31.9526977539063, true) <= 5 then
                if IsPedInModel(GetPlayerPed(-1), GetHashKey("bodhi2")) then
                    autorisation_chasse = 1
                else
                    autorisation_chasse = 0
                end
                if autorisation_chasse == 1 then
                    if(exports["vdk_inventory"]:getQuantity(23) > 0) then
                        VenteViande(1)
                    else
                        VenteViande(0)
                    end
