...
Weeds.tranformCoord = {x = -2.16, y = -1496.59, z =  31.85}
...
Weeds.Text = {
    ...
    RecolteKo = '~o~La pousse n\'est pas encore à maturitée',
    RecoltePremature = '~b~Recolte prémature possible',
    ...
    NoPassword = '...',
    PasswordKo = '~r~C\'est pas le bon mot de passe, dégage ou ça mal se passer pour toi !',
    PasswordOld  = '~b~C\'est l\'ancien mot de passe ça ! t\'es un flic ?!',
    PasswordCorrect = '~g~Je t\'achètes tous pour un total de ~r~',
    GoodPlaceToSell = '~o~C\'est un bon emplacement pour vendre des joins, trouves des clients',
    PasswordSend = "Près de ~b~la boutique~w~, quelqu'un achète en masse.\n~o~Mot de passe: ~r~",
    Buy2 = '~g~Je te prends deux ~b~joins~w~ pour ~o~',
    Buy1 = '~b~Je te prends un ~b~join~w~ pour ~o~',
    Buy0 = '~o~Garde ta merde, j\'en veux pas',
    DejaPropo = '~d~Encore ?, Va voir ailleurs, j\'ai déjà ce qu\'y m\'faut~',
...
Weeds.systemGrowth = function (id, seed)
    if seed.time == 0 then
        ...
        local d = math.floor((getTimes()-seed.time) / WeedGrowthTime * 10000) / 100
        if d < 50 then
        ...
        if IsControlJustPressed(1, 38) then 
           ...
           -- local ped = GetRandomPedAtCoord(myPos.x, myPos.y, myPos.z, 3.0, 3.0, 3.0, -1)
            if ped ~= 0 then
                ...
                if lastBuy == 0 or lastBuy + PNJ_TIME_SELL < getTimes() then
                    local r = math.random() 
                    ...
                    elseif r > 0.9 and totalItem > 2 then
						...
						-- end
                        -- local total = math.random((JoinVenteUnite[1]*(1.2^nbPolice)), (JoinVenteUnite[2]*(1.2^nbPolice)))
						local total = math.random(JoinVenteUnite[1], JoinVenteUnite[2]) * (1.2 ^ nbPolice)
                    ...
                    DecorSetInt(ped, 'illegal_lastBuyWeed', getTimes())
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5000)
        if NearWeed then
            TriggerServerEvent('illegal:getTimes')
            NearWeed = false
        end
    end
end)
...
--  Event
--=============================================================================
RegisterNetEvent('illegal:illegalsetcop')
...
AddEventHandler('illegal:setFullPlantData',function(data,ostime)
    osservtime = ostime
...
RegisterNetEvent('illegal:setTimes')
AddEventHandler('illegal:setTimes',function(time)
    osservtime = time
end)
...
AddEventHandler('illegal:fullVente', function( sta, qte)
    ...
    elseif sta == 1 then
		...
		-- for i=0,nbPolice,1 do
			-- pourcentage = pourcentage * 1.2
		-- end
        local total = (qte * JoinVenteFull) * 1.2
...
--[[Citizen.CreateThread(function()
	while true do
		Citizen.Wait(30000)
		TriggerServerEvent('weed:testcop')
	end
end)]]--
