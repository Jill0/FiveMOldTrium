...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("Appuyez sur ~g~~INPUT_CONTEXT~~s~ pour arrêter de jouer au golf.")
			else
				DisplayHelpText("Appuyez sur ~g~~INPUT_CONTEXT~~s~ pour commencer à jouer au golf. ($100)")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					Citizen.Trace("a1")
					TriggerServerEvent("golf:buy")
					spawnCart()
					startGolf() -- If you plan to have it cost money, you need to remove this and only call it when they paid
					TriggerEvent("customNotification","Appuyez sur E pour faire un swing, Q-D pour diriger, Y pour changer de club.")
				end
			end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Déplacez vous vers votre balle, Appuyez sur ~g~E~s~ si la balle est coincé.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if distance < 1.5 then
		TriggerEvent("customNotification","Vous avez la balle à portée !")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Vous avez fait trop de tirs ..")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Appuyez sur ~g~E~s~ pour lâcher la balle ici.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Appuyez sur ~g~E~s~ pour lâcher - ~r~ trop loin de la balle ou pour fermer le trou.")
		end
	end
end
...
function addBallBlip()
	...
	AddTextComponentString(tostring("Balle"))
...
function BlipsStartEnd()
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Début du trou"))
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Fin du trou"))
...
function addblipGC()
	...
	BeginTextCommandSetBlipName("STRING");
	AddTextComponentString(tostring("Terrain de golf"))
