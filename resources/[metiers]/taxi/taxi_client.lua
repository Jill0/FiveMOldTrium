...
AddEventHandler('taxi:receiveIsTaxi', function(result)
  Citizen.Trace('isTaxi')
  if(result == '0') then
    isTaxi = false
	print("VOUS N'ETES PAS TAXIMAN!")
  else
    isTaxi = true
	print("VOUS ETES TAXIMAN!")
  end
end)
...
local function IAgotoPlayer(type)
  ...
  while not IsPointOnRoad(coords.x, coords.y, spawnZ, 0) do --or isok == false or isfree ~= 0 do --last param is useless https://wiki.rage.mp/index.php?title=Pathfind::isPointOnRoad
...
--  SetDriverAbility(IAdriver, 0.99
 ...
 --TaskVehicleDriveToCoord(IAdriver, IAcar, myPos.x, myPos.y, myPos.z, 80.001, 1.1, GetHashKey(vehicle), 1024, 10.0001, true) --http://gtaforums.com/topic/822314-guide-driving-styles/
--  Citizen.InvokeNative(0x158BB33F920D360C, IAdriver, IAcar, myPos.x, myPos.y, myPos.z, 80.001, 1.1, GetHashKey(vehicle), 786859, 10.0001, true) --http://gtaforums.com/topic/822314-guide-driving-styles/
	Citizen.InvokeNative(0x158BB33F920D360C, IAdriver, IAcar, myPos.x, myPos.y, myPos.z, 10.01, 2621445, 20.0001)
	--SetDriveTaskDrivingStyle(IAdriver,1024)
end
...
--  TaskVehicleDriveToCoord(IAdriver, IAcar, IATarget.x, IATarget.y, IATarget.z, 80.001, 1.1, GetHashKey(vehicle), 786859, 20.0001, true) -- task hash : 0x93a5526e (https://pastebin.com/R9iK6M9W)
  Citizen.InvokeNative(0x158BB33F920D360C, IAdriver, IAcar, IATarget.x, IATarget.y, IATarget.z, 20.0, 2621445, 20.0001) -- task hash : 0x21d33957 (https://pastebin.com/R9iK6M9W)
  --SetDriveTaskDrivingStyle(IAdriver,1024)
	--Citizen.InvokeNative(0x158BB33F920D360C, IAdriver, IAcar, IATarget.x, IATarget.y, IATarget.z, 80.001, 1.1, GetHashKey(vehicle), 1074528293, 10.0001, true) --http://gtaforums.com/topic/822314-guide-driving-styles/
	--TaskVehicleDriveToCoord(IAdriver, IAcar, IATarget.x, IATarget.y, IATarget.z, 80.001, 1.1, GetHashKey(vehicle), 1024, 10.0001, true) --http://gtaforums.com/topic/822314-guide-driving-styles/
...
--      if not IAfoundZ then
        if GetDistanceBetweenCoords(taxiPos.x, taxiPos.y, IATarget.z, IATarget.x, IATarget.y, IATarget.z, true) <= 10 then
...
--      end
    ...
    elseif IAstate == 5 then
      ...
      startpaying = false
      TaskVehicleDriveWander(IAdriver, IAcar, 20.0, 1074528293)
      taxi_call_accept = 0
      IAstate = 0
...
-- Citizen.CreateThread(function()
  -- while true do
    -- Citizen.Wait(10000)
    -- if startpaying then
      -- TriggerServerEvent('taxi:PayAutomaticTaxi', 25)
    -- end
  -- end
-- end)
--====================================================================================
...
function spawnVehicle(coords, type)
	for _, pos in pairs(coords) do
	    if pos.type == type then
	      local vehi = GetClosestVehicle(pos.x, pos.y, pos.z, 4.0, 0, 70)
...
local function toogleService()
  ...
  if inService then
	...
	TriggerServerEvent('taxi:setService',true)
    TriggerServerEvent('taxi:requestMission')
  else
    -- Restaure Ped
    TriggerServerEvent('taxi:endService')
	TriggerServerEvent('taxi:setService',false)
...
Citizen.CreateThread(function()
  while true do
    ...
    if taxi_call_accept ~= 0 and not statusHUD then
      showInfoClient()
    end
  end
end)
