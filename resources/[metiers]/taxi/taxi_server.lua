...
RegisterServerEvent('taxi:setService')
	AddEventHandler('taxi:setService', function (inService)
		TriggerEvent('es:getPlayerFromId', source , function (Player)
			Player.func.setSessionVar('taxiInService', inService)
		end)
	end)
...
AddEventHandler('taxi:checkIsTaxi', function()
	TriggerEvent("es:getPlayerFromId", source, function(user)
		...
		print(identifier)
		checkIsTaxi(identifier, user.source)
	end)
end)

function checkIsTaxi(identifier, source)
	--[[local result = MySQL.Sync.fetchAll('SELECT job FROM users WHERE identifier = @identifier AND job=17', { ['@identifier'] = identifier})
	--print("TOTO")
	--print('----Result' .. result[1].rank .. '----')
	if(not result[1]) then
		TriggerClientEvent('taxi:receiveIsTaxi', source, '0')
	else
		TriggerClientEvent('taxi:receiveIsTaxi', source, '1')
	end]]--
	TriggerEvent("es:getPlayerFromId", source, function(user)
		if(user.jobId ~= 17) then
			TriggerClientEvent('taxi:receiveIsTaxi', source, '0')
		else
			TriggerClientEvent('taxi:receiveIsTaxi', source, '1')
		end
