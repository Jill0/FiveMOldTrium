...
AddEventHandler("playerSpawned", function()
    TriggerServerEvent('watermark:getServer')
end)

RegisterNetEvent('watermark:displayServer')
AddEventHandler('watermark:displayServer', function(result)
    if (result) then
		result = string.sub(result, 8)
        servername = "Ville"..result.." | ~r~Trium Roleplay~s~ | discord.me/trium"
    end
end)
