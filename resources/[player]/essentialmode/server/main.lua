...
AddEventHandler('playerDropped', function()
    ...
    if(Users[mysource])then
        -- Suppression des urgences
        exports["c_services"]:RemoveEmergencyConnected(Users[mysource].jobId)
        --
        Users[mysource].func.disconnect()
        MySQL.Async.execute("UPDATE users SET `money`=@value, `dirty_money`=@v2 WHERE `identifier` = @identifier", 
        {['@value'] = Users[mysource].money, ['@v2'] = Users[mysource].dirty_money, ['@identifier'] = Users[mysource].identifier})
        TriggerEvent("es:playerDropped", Users[mysource])
        Users[mysource] = nil
    end
end)
...
AddEventHandler('es:updatePositions', function(x, y, z)
    if(Users[source])then
        Users[source].func.setCoords(x, y, z)
    end
end)
