...
function checkVeh()
	local isGoodVehcile = true
	myPed = GetPlayerPed(-1)
	currentVehicle = GetVehiclePedIsIn(myPed, false)
    if currentVehicle ~= 0 then
		if (IsVehicleModel(currentVehicle,GetHashKey('journey')) or IsVehicleModel(currentVehicle,GetHashKey('surfer')) or IsVehicleModel(currentVehicle,GetHashKey('surfer2')) or IsVehicleModel(currentVehicle,GetHashKey('camper'))) then
			isGoodVehcile = true 
		else
			isGoodVehcile = false 
		end
	...
	--print("vehicule is :"..isGoodVehcile)
	return isGoodVehcile
end
...
function recolt(text, rl)
    ...
    elseif (text == 'Traitement') then
		local IsLabo= checkVeh()
		
		if ((JOBS[jobId].raw_item =='Methylamine' and IsLabo==true ) or JOBS[jobId].raw_item~='Methylamine') then 
		...
		elseif (JOBS[jobId].raw_item =='Methylamine' and IsLabo==false) then
			TriggerEvent("mt:missiontext","~w~Cet emplacement serait parfait pour installer ~g~un labo mobile ~w~...", timeForRecolt - 800)
