...
AddEventHandler("barber:getOldSkin", function()
    TriggerEvent('es:getPlayerFromId', source, function(user)
        ...
        TriggerClientEvent('barber:getOldSkinFromServer', user.source, skin)
    end)
end)
...
AddEventHandler('barber:pay', function(price, newSkin)
    TriggerEvent('es:getPlayerFromId', source, function(user)
        if tonumber(user.money) > tonumber(price) and tonumber(user.money) > 0 then
            ...
            TriggerClientEvent('barber:closeMenu', user.source, {transaction = true, price = price})
            ...
            TriggerClientEvent('barber:closeMenu', user.source, {transaction = false, price = price})
        end
    end)
end)
...
AddEventHandler('barber:saveHeadSkin', function(player, newSkin)
    ...
    if newSkin.hair_color then
        saveHeadSkin(player, { type = 'hair_colour', val = newSkin.hair_color })
    end
...
function getCurrentSkin(player)
    local skin = MySQL.Sync.fetchAll("SELECT * FROM modelmenu WHERE identifier = @username", {['@username'] = player})
    if skin then
        return skin
    end
end
...
function saveHeadSkin(player,skin)
    MySQL.Async.execute("UPDATE modelmenu SET "..skin.type.."=@a WHERE `identifier`=@id",
        {
            ['@id'] = player,
            ['@a']  = tonumber(skin.val)
        })
