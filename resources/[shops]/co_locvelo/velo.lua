...
local velshop = {
	...
	menu = {
		...
		["main"] = { 
			...
			name = "main",
			buttons = { 
				{name = "Vélos Classique", description = ''},
				{name = "Vélo de Sport", description = ''},
			}
		},
		["vehicles"] = { 
			title = "VEHICLES", 
		...
		["Velo-Classique"] = { 
			...
			buttons = { 
				...
				{name = "VTT", costs = 20, description = {}, model = "scorcher"},
				{name = "Bicyclette", costs = 20, description = {}, model = "cruiser"},			
			}
		},
		["Velo-Sport"] = { 
			title = "Velo-Sport", 
			name = "Velo-Sport",
			buttons = { 
				{name = "Vélo sport", costs = 20, description = {}, model = "fixter"},
				{name = "Vélo course 1", costs = 20, description = {}, model = "tribike"},
				{name = "Vélo course 2", costs = 20, description = {}, model = "tribike2"},
				{name = "Vélo course 3", costs = 20, description = {}, model = "tribike3"},
			}
		},
	}
}
...
{entering = {176.955,-910.295,29.692}, inside = {179.979,-908.462,29.692,120.1953}, outside = {176.955,-910.295,29.692}}, -- Garage 3/3 Stunt Place
...
{entering = {-679.16, 5797.33, 16.33},inside = {-679.16, 5797.33, 17.00},outside  = {-679.16, 5797.33, 16.33}},
}
...
function StuntVeloBlip()
    local blip = AddBlipForCoord(176.955,-910.295,29.692)
    SetBlipSprite(blip,435)
    SetBlipColour(blip, 3)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString('[Activité] Stunt Vélo')
		...
		Citizen.CreateThread(function()
			while #velshop_blips > 0 do
				...
				for i,b in ipairs(velshop_blips) do
					if IsPlayerWantedLevelGreater(GetPlayerIndex(),0) == false and velshop.opened == false and IsPedInAnyVehicle(LocalPed(), true) == false and  GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed())) < 5.0 then
						DrawMarker(1,b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],0,0,0,0,0,0,2.001,2.0001,0.5001,0,155,255,200,0,0,0,0)
					end
					if IsPlayerWantedLevelGreater(GetPlayerIndex(),0) == false and velshop.opened == false and IsPedInAnyVehicle(LocalPed(), true) == false and  GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed())) < 0.4001 then
						drawTxt('Appuyez sur ~g~ENTER~s~ pour acceder au ~b~vélos',0,1,0.5,0.8,0.6,255,255,255,255)
			...
			for i,button in pairs(menu.buttons) do
				if i >= velshop.menu.from and i <= velshop.menu.to then
					...
					if button.costs ~= nil then
						if velshop.currentmenu == "Velo-Classique" or velshop.currentmenu == "Velo-Sport" then
...
function ButtonSelected(button)
	...
	if this == "main" then
		if btn == "Vélos Classique" then
			OpenMenu('Velo-Classique')
		elseif btn == "Vélo de Sport" then
			OpenMenu('Velo-Sport')
		end
		
	elseif this == "Velo-Classique" or this == "Velo-Sport" then
...
function Back()
	...
	if velshop.currentmenu == "main" then
		CloseCreatorvelo()
	elseif velshop.currentmenu == "Velo-Classique" or velshop.currentmenu == "Velo-Sport" then
		if DoesEntityExist(fakevelo.velo) then
			Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(fakevelo.velo))
		end
		fakevelo = {model = '', velo = nil}
