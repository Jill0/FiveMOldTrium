...
local function DrawPlayerList()
	...
	for k, v in pairs( players ) do
		...
		if Settings["PlayerID"] then
			if (scoreboardIsAdmin == false) then
				local pid = GetPlayerServerId(v)
				AddTextComponentString(pid .. " ##########")
			elseif (scoreboardIsAdmin == true) then
				local pid = GetPlayerServerId(v)
				AddTextComponentString(pid .. " " ..  GetPlayerName(pid))
			end
	   else
...
RegisterNetEvent("scoreboard:setAdmin")
AddEventHandler("scoreboard:setAdmin", function(val)
  if val == 'admin' then
    scoreboardIsAdmin = true
