...
local function SpawnVehicle(model, x, y, z, heading, ped)
	...
	if IsModelValid(model) then
		...
		TriggerServerEvent('mellotrainer:VehiMsg',model)	
...
RegisterNUICallback("veh", function(data, cb)
	...
	if playerVeh <= 0  and action ~= "delete" and action ~= "unlock" then
		drawNotification("~r~Not in a vehicle!")
		return
	end
	...
	elseif action == "lock"then
		...
		drawNotification("~g~Véhicule verrouillé.")
	...
	elseif action == "unlock"then
        ...
        if ( DoesEntityExist( vehicle ) ) then
			...
			drawNotification("~g~Vehicule déverrouillé.")
		end
    ...
    if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then 
        ...
        if ( IsPedSittingInAnyVehicle( ped ) ) then 
            ...
            if ( GetPedInVehicleSeat( vehicle, -1 ) == ped ) then 
                drawNotification( "Vehicule supprime." )
                SetEntityAsMissionEntity( vehicle, true, true )
                Citizen.InvokeNative( 0xEA386986E786A54F, Citizen.PointerValueIntInitialized( vehicle ) )
            else 
                drawNotification( "Mettez-vous à la place conducteur" )
            end 
        else
            local playerPos = GetEntityCoords( ped, 1 )
            local inFrontOfPlayer = GetOffsetFromEntityInWorldCoords( ped, 0.0, distanceToCheck, 0.0 )
            ...
            if ( DoesEntityExist( vehicle ) ) then 
                ...
                drawNotification( "Rapprochez-vous d'un vehicule" )
            end 
        end 
    end 
		drawNotification("~g~Vehicle supprimé")
