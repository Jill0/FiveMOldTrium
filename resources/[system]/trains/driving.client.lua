	...
	if Config.ModelsLoaded then
		...
		if (Config.inTrain) then
			...
			elseif (IsControlPressed(0,73)) then -- E Break (X)
				Config.Speed = 0
			elseif (IsControlPressed(0,44) and Config.Speed == 0) then -- Open/Close doors (A)
				Config.CloseDoors = not Config.CloseDoors
				Citizen.Wait(200)
			elseif (IsControlPressed(0,71) and Config.Speed < getTrainSpeeds(Config.TrainVeh).MaxSpeed and Config.CloseDoors) then  -- Forward (W)
				debugLog("W: " .. Config.Speed)
				Config.Speed = Config.Speed + getTrainSpeeds(Config.TrainVeh).Accel
			elseif (IsControlPressed(0,72) and Config.Speed  > -getTrainSpeeds(Config.TrainVeh).MaxSpeed and Config.CloseDoors) then -- Backwards (S)
...
function portesclose()
	if Config.inTrain and Config.CloseDoors then
		SetVehicleDoorShut(Config.TrainVeh, 0, false)
		SetVehicleDoorShut(Config.TrainVeh, 1, false)
		SetVehicleDoorShut(Config.TrainVeh, 2, false)
		SetVehicleDoorShut(Config.TrainVeh, 3, false)
--		elseif Config.inTrain then
--		SetVehicleDoorOpen(Config.TrainVeh, 0, false)
--		SetVehicleDoorOpen(Config.TrainVeh, 1, false)
--		SetVehicleDoorOpen(Config.TrainVeh, 2, false)
--		SetVehicleDoorOpen(Config.TrainVeh, 3, false)
	end
end
