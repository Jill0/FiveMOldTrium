...
CONTROLS
	...
	Hazard lights:	\|/	(Phone Cancel)
	Toggle emergency lights:	1	(Text Chat Team)
	Airhorn:	E	(Horn)
	Toggle siren:	2	(Previous Radio Station)
	Manual siren / Change siren tone:	3	(Next Radio Station)
	Auxiliary siren:	4	(Phone Up)
---------------------------------------------------
]]
...
function SetLightsStateForVeh(veh, newstate)
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if newstate == 0 and state_light[veh] ~= 0 then
			SetVehicleSiren(veh, false)
		elseif newstate == 1 and state_light[veh] ~= 1 then
			SetVehicleSiren(veh, true)
		end
		state_light[veh] = newstate
	end
end
...
---------------------------------------------------------------------
RegisterNetEvent("lvc_TogDfltSrnLights_c")
AddEventHandler("lvc_TogDfltSrnLights_c", function(sender, newstate)
	...
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
			if IsPedInAnyVehicle(ped_s, false) then
				...
				if GetPedInVehicleSeat(veh, -1) == ped_s then
			...
			if IsPedInAnyVehicle(playerped, false) then
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						DisableControlAction(0, 164, true) -- INPUT_CELLPHONE_UP
						DisableControlAction(0, 158, true) -- INPUT_VEH_PREV_RADIO
						DisableControlAction(0, 19, true) -- INPUT_CHARACTER_WHEEL
						DisableControlAction(0, 157, true) -- INPUT_VEH_RADIO_WHEEL
						DisableControlAction(0, 160, true) -- INPUT_VEH_CIN_CAM
						...
						if not IsPauseMenuActive() then
							...
							if state_lxsiren[veh] < 1 then
								if IsDisabledControlPressed(0, 160) then
									actv_manu = true
								else
									actv_manu = false
								end
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if not IsPauseMenuActive() then
							...
							-- IND L
							if IsControlJustPressed(0, 174) and GetLastInputMethod(2) and dontuseable == false then -- INPUT_VEH_PREV_RADIO_TRACK
								...
								count_bcast_timer = delay_bcast_timer
							-- IND R
							elseif IsControlJustPressed(0, 175) and GetLastInputMethod(2) and dontuseable == false then -- INPUT_VEH_NEXT_RADIO_TRACK
								local cstate = state_indic[veh]
								...
								count_bcast_timer = delay_bcast_timer
							-- IND H
							elseif IsControlJustPressed(0, 173) and GetLastInputMethod(2) and dontuseable == false then -- INPUT_FRONTEND_CANCEL / Backspace
						...
						if count_bcast_timer > delay_bcast_timer then
							...
							--- IS EMERG VEHICLE ---
							if GetVehicleClass(veh) == 18 then
								TriggerServerEvent("lvc_TogDfltSrnLights_s", state_light[veh])
...
AddEventHandler('indicators:dontuseable', function(Toggle)
	Citizen.CreateThread(function()
