...
function CloseVehiclesInventory()
    if(not Menu.hidden or vehMenuIsOpen) then
        Menu.hidden = true
        vehMenuIsOpen = false
    end
end
...
function GetItem(arg)
    ...
    if vehFront > 0 then
        ...
        if qtyn ~= nil and qtyn >= 1 then
            ...
            if(exports["vdk_inventory"]:notFull() == true and exports["vdk_inventory"]:tryFull(qty) == true) then
                ...
                TriggerEvent("ft_libs:AdvancedNotification", {
                    ...
                    title = "iTrium",
                    text = "Vous n'avez pas assez de place sur vous !"
                })
            end
        end
    end
        ...
        if qtyn ~= nil and qtyn >= 1 then
            ...
            if(qty <= maxcap) then
                ...
                TriggerEvent("ft_libs:AdvancedNotification", {
                    icon = "CHAR_MP_STRIPCLUB_PR",
                    title = "iTrium",
                    text = "Il n'y a pas assez de place dans le véhicule"
                })
            end
        end
    end
